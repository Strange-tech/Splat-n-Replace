...
class Scene:
    ...
    def __init__(
        ...
        if load_iteration:
            if load_iteration == -1:
                if mode == "train":
                    ...
                    if target == "seg" or target == "coarse_seg_everything":
                        self.feature_loaded_iter = (
                            ...
                            if (
                                feature_load_iteration is None
                                or feature_load_iteration == -1
                            )
                            else feature_load_iteration
                        )
                        self.loaded_iter = searchForMaxIteration(
                            os.path.join(self.model_path, "point_cloud"), target="scene"
                        )
                    elif target == "scene":
                        self.feature_loaded_iter = None
                        self.loaded_iter = searchForMaxIteration(
                    ...
                    if target == "seg":
                        ...
                        self.loaded_iter = searchForMaxIteration(
                            os.path.join(self.model_path, "point_cloud"), target="seg"
                        )
                    elif target == "scene":
                        self.feature_gaussians = None
                        self.feature_loaded_iter = None
                        ...
                        self.loaded_iter = (
                            ...
                            else searchForMaxIteration(
                                os.path.join(self.model_path, "point_cloud"),
                                target="scene",
                            )
                        )
                    elif target == "coarse_seg_everything":
                        self.feature_loaded_iter = searchForMaxIteration(
                            os.path.join(self.model_path, "point_cloud"), target=target
                        )
                        self.loaded_iter = searchForMaxIteration(
                            os.path.join(self.model_path, "point_cloud"), target="scene"
                        )
                ...
                if mode == "train":
                    ...
                    if target == "seg":
                        ...
                        target == "feature"
                        or target == "coarse_seg_everything"
                        or target == "contrastive_feature"
                    ):
                        self.feature_loaded_iter = (
                            searchForMaxIteration(
                                os.path.join(self.model_path, "point_cloud"),
            ...
            print(
                "Loading trained model at iteration {}, {}".format(
                    self.loaded_iter, self.feature_loaded_iter
                )
            )
        ...
        if self.feature_loaded_iter and self.feature_gaussians is not None:
            if target == "feature" or target == "seg":
                self.feature_gaussians.load_ply(
                    os.path.join(
                        self.model_path,
                        "point_cloud",
                ...
                if mode == "train":
                    self.feature_gaussians.load_ply_from_3dgs(
                        os.path.join(
                            self.model_path,
                            "point_cloud",
                            "iteration_" + str(self.loaded_iter),
                    ...
                    fetchPly(
                        os.path.join(
                            ...
                            + str(
                                ...
                                if (self.loaded_iter is None or self.loaded_iter == -1)
                                else self.loaded_iter
                            ),
                            "scene_point_cloud.ply",
                        ),
                        only_xyz=True,
    ...
    def save(self, iteration, target="scene"):
        assert (
            target != "feature"
            and "Please use save_feature() to save feature gaussians!"
        )
        point_cloud_path = os.path.join(
            self.model_path, "point_cloud/iteration_{}".format(iteration)
        )
        self.gaussians.save_ply(
            os.path.join(point_cloud_path, target + "_point_cloud.ply")
        )
    ...
    def save_mask(self, iteration, id=0):
        ...
        self.gaussians.save_mask(
            os.path.join(point_cloud_path, f"seg_point_cloud_{id}.npy")
        )

    def save_feature(
        self,
        iteration,
        target="coarse_seg_everything",
        smooth_weights=None,
        smooth_type=None,
        smooth_K=16,
    ):
        assert self.feature_gaussians is not None and (
            target == "feature"
            or target == "coarse_seg_everything"
            or target == "contrastive_feature"
        )
        ...
        self.feature_gaussians.save_ply(
            os.path.join(point_cloud_path, f"{target}_point_cloud.ply"),
            smooth_weights,
            smooth_type,
            smooth_K,
        )
