...
class CONFIG:
    r = 2  # scale ratio
    window_width = int(3840 / r)
    window_height = int(2160 / r)
...
class GaussianSplattingGUI:
    def __init__(
        ...
        feature_gaussian_model: FeatureGaussianModel = None,
        scale_gate: torch.nn.modules.container.Sequential = None,
    ) -> None:
        self.opt = opt
    ...
    def fetch_data(self, view_camera):
        ...
        if self.save_flag:
            ...
            #     )
            #     torch.save(
            #         save_mask, f"./segmentation_res/{dpg.get_value('save_name')}.pt"
            #     )
            # except:
