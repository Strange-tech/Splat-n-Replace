...
def random_point_sampling(xyz, n_samples, replace=False):
    ...
    """
    N = xyz.shape[0]
    idx = torch.randperm(N, device=xyz.device)[:n_samples] if not replace else \
          torch.randint(0, N, (n_samples,), device=xyz.device)
    return idx