...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        for i, image_path in tqdm(enumerate(sorted(os.listdir(os.path.join(args.image_root, 'images'))))):
            ...
            masks = torch.load(os.path.join(os.path.join(args.image_root, 'grounded_sam_masks'), image_path.replace('jpg', 'pt').replace('JPG', 'pt').replace('png', 'pt')))
            # N_mask, C
            features = get_features_from_image_and_masks(clip_model, image, image_path, masks, background = 0.)
