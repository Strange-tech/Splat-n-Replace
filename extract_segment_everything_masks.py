...
VIDEO_PATH = os.path.join('./', "segmentation_visualization.mp4")
...
if __name__ == '__main__':
    ...
    OUTPUT_TXT_DIR = os.path.join(args.image_root, 'sam_masks_txt')
    ...
    for path in tqdm(sorted(os.listdir(IMAGE_DIR))):
        ...
        for m in masks:
            ...
            if len(m_score.unique()) < 2:
                ...
                # mask 可视化叠加
                mask_rgb = np.stack([m_score.detach().cpu().numpy().astype(np.uint8) * np.random.randint(0, 255)]*3, axis=-1)
                color_mask = cv2.add(color_mask, mask_rgb)
        masks = torch.stack(mask_list, dim=0)

        torch.save(masks, os.path.join(OUTPUT_DIR, name+'.pt'))

        # np.savetxt(os.path.join(OUTPUT_TXT_DIR, name+'.txt'), mask_rgb.reshape(-1, 3))

        # 叠加原图与 mask
        visual = cv2.addWeighted(img, 0.1, color_mask, 0.9, 0)
