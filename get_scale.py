...
if __name__ == "__main__":
    ...
    for i, image_path in tqdm(
        enumerate(sorted(os.listdir(os.path.join(dataset.source_path, "images"))))
    ):
        # print(image_path)
        image = cv2.imread(
            os.path.join(os.path.join(dataset.source_path, "images"), image_path)
        )
    ...
    for it, view in tqdm(enumerate(cameras)):
        ...
        for mask_id in range(len(corresponding_masks)):
            ...
            if torch.isnan(scale[mask_id]) or torch.isinf(scale[mask_id]):
                print(
                    f"Warning: Scale for mask {mask_id} in image {view.image_name} is NaN or Inf, setting to 0."
                )
                scale[mask_id] = 0.0
